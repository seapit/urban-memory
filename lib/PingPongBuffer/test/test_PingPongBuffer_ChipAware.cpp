...
// (in lib/PingPongBuffer/CMakeLists.txt) is populated - that's what gates this, not
// a comment block: test/CMakeLists.txt's if(CHIPS) branch is the only
// thing that ever references this file.
#include <gtest/gtest.h>

#include "PingPongBuffer/Config.hpp"

TEST(PingPongBufferTest, ChipConfigHeaderIsReachable) {
  ...
  EXPECT_GE(PingPongBuffer::kMaxRetries, 0);
}
